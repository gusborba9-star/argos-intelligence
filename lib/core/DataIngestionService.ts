...
// ============================================================
// DATA INGESTION SERVICE v5.6.0 — SINGLE-PASS ARCHITECTURE
// ============================================================
...
export interface FreshnessResult {
  ...
  known: boolean;
  checkedAt: string;
  reason: "CHANGED" | "UNCHANGED" | "UNKNOWN" | "ERROR";
}
...
export class DataIngestionService {
  ...
  private static readonly ANALYSIS_HORIZON_MS = 48 * 60 * 60 * 1000;

  constructor() {
    this.apiKey = process.env.PROPLINE_API_KEY || "";
    console.log("Versão do Argos: 5.6.0 - SINGLE-PASS ARCHITECTURE / 48H EVIDENCE WINDOW");
  }
  ...
  public async getActiveSports(): Promise<any[]> {
    try {
      ...
      return rawSports.filter((s: any) => s.active && s.key.toLowerCase().includes("soccer"));
    } catch (error: any) {
  ...
  /**
   * Freshness is tri-state internally. A provider response with changed=false
   * must never be coerced to true. Errors are observable and conservatively
   * represented as unknown rather than pretending that data changed.
   */
  public async getFreshness(sportKey: string): Promise<FreshnessResult> {
    ...
    try {
      ...
      if (typeof changed !== "boolean") {
        return { changed: false, known: false, checkedAt, reason: "UNKNOWN" };
      }
  ...
  /** Backward-compatible boolean API. Unknown/error is false, never an implicit true. */
  ...
  public async getMegaCallOdds(sportKey: string): Promise<any[]> {
    try {
      ...
      console.log(`[Argos-URL] Mega Call All-In v5.6.0: ${url.replace(this.apiKey, "***")}`);
      ...
      const filtered = events.filter((e: any) => {
        ...
        return validKickoff && kickoff > now && kickoff <= horizon && hasOdds;
      });
  ...
  public async saveMatchToDatabase(fixture: any): Promise<void> {
    try {
      ...
      if (!Number.isFinite(kickoffMs)) throw new Error(`Invalid kickoff for match ${matchId}`);
  ...
  async ingestObject(fixture: any): Promise<IngestedData> {
    ...
    if (commenceTime > now + DataIngestionService.ANALYSIS_HORIZON_MS) {
      throw new Error("OUTSIDE_ANALYSIS_HORIZON - Evento fora da janela canônica de 48h.");
    }
  ...
  public async getCachedMatchData(matchId: string): Promise<{ rawData: any } | null> {
    try {
      const { data, error } = await this.supabase
        .from("argos_matches")
        .select("raw_data, kickoff_at, updated_at")
        .eq("match_id", matchId)
      ...
      const now = Date.now();
      if (!Number.isFinite(kickoffMs) || !Number.isFinite(updatedMs) || kickoffMs <= now || kickoffMs > now + DataIngestionService.ANALYSIS_HORIZON_MS) {
        console.warn(`[Argos-Cache] Payload ${matchId} rejeitado por horizonte temporal.`);
        return null;
      }
  ...
  public async updateTeamFormFromScores(sportKey: string, daysFrom: number = 3): Promise<number> {
    try {
      ...
      for (const ev of events) {
        ...
        if (ev.home_score === null || ev.home_score === undefined || ev.away_score === null || ev.away_score === undefined) continue;
  ...
  public async getRealTeamHistory(sportKey: string, teamName: string): Promise<{ goals: { home: number; away: number } }[]> {
    const canonicalName = normalizeTeamName(teamName);
    const { data } = await this.supabase.from("argos_team_form").select("recent_goals_for, recent_goals_against").eq("sport_key", sportKey).eq("team_name", canonicalName).maybeSingle();
  ...
  public async getTeamExtraStats(sportKey: string, teamName: string): Promise<{ cornersFor: number; cornersAgainst: number; cardsFor: number; cardsAgainst: number; shotsFor: number; shotsAgainst: number; sampleSize: number } | null> {
    const canonicalName = normalizeTeamName(teamName);
    const { data } = await this.supabase.from("argos_team_form").select("recent_corners_for, recent_corners_against, recent_cards_for, recent_cards_against, recent_shots_for, recent_shots_against").eq("sport_key", sportKey).eq("team_name", canonicalName).maybeSingle();
