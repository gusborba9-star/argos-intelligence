...
export class FeatureEngine {
  private static readonly GOAL_PRIOR = 1.35;
  private static readonly GOAL_PRIOR_STRENGTH = 4;
    ...
    return {
      ...
      leagueProfile: this.normalizeLeagueProfile(
        rawData.fixture?.league ?? rawData.league ?? {
          name: rawData.sport_title || rawData.league_name || (rawData.sport_key ? rawData.sport_key.replace(/^soccer_/, "").replace(/_/g, " ").replace(/\b\w/g, (c: string) => c.toUpperCase()) : "Unknown League"),
  ...
  private static extractSideStatistics(stats: any[], side: "home" | "away") {
    const teamBlock = stats.find((entry: any) => entry?.team?.name);
    const source = Array.isArray(teamBlock?.statistics) ? teamBlock.statistics : stats;
  ...
  private static normalizeLeagueProfile(league: any) {
    ...
    return { tier: normalizedTier, country: league?.country ?? "Global", name: league?.name ?? "Unknown League" };
  }
}
